...
Define(RandomSymmetric)
{
    It("Generates a 2x2 symmetric matrix")
    {
        ...
        std::shared_ptr<const matrix<double>> result = random_symmetric_command.call(rows, 1).get_value();

        transpose transpose_command;

        AssertElementsEqual(*result, *transpose_command.call(result, 1).get_value());
    } Done
    ...
    It("Throws an exception when called with an input of a size that is not 1,1")
    {
        ...
        try
        {
            random_symmetric_command.call(matrix1, 1).get_value();
        }
